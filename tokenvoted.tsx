  ...
  const fetchFirestoreTokens = async () => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        console.log("Token data:", doc.data());
        tokens.push({
          id: doc.id,
          address: doc.data().address,
          name: doc.data().name,
          liquidity: doc.data().liquidity || 0,
          starred: doc.data().starred || false,
          symbol: doc.data().symbol,
          launchDate: doc.data().launchDate,
          launcherAddress: doc.data().launcherAddress,
        } as FirestoreToken);
      });
    ...
    try {
      ...
      for (const token of firestoreTokens) {
        ...
        newTokenPools.push({
          ...
          active: votingActive,
          logo: null, // Cambiado a null para evitar el error 404

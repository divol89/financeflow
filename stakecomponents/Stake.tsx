...
export default function Stake() {
  ...
  if (rewardRatio) {
    rewardNumerator = parseFloat(
      ethers.utils.formatEther(rewardRatio._numerator),
    );
    rewardDenominator = parseFloat(
      ethers.utils.formatEther(rewardRatio._denominator),
    );
  ...
  const totalStakedFloat = totalStaked
  ...
  const userStakeProportion = totalStakedFloat
    ? userStake / totalStakedFloat
    : 0;
  ...
  const individualAPR = userStake
    ? ((dailyRewardForUser * 365) / userStake) * 100
    : 0;
  ...
  return (
    <Card
      className="bg-gradient-to-t from-orange-500 to-gray-300"
      p={5}
      mt={10}
    >
      <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
        ...
        <Card p={5} m={5}>
          ...
          >
            ...
            <Text>User Stake Proportion: {roundedStakeProportion}</Text>
          </Flex>
        </Card>
      </SimpleGrid>
    </Card>

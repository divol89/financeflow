      ...
      <header className={`levi-header ${scrolled ? "is-scrolled" : ""}`}>
        <div className="levi-container flex h-[4.75rem] items-center justify-between">
          ...
          {auth.session || auth.walletAddress ? (
            ...
            <button type="button" onClick={() => setAccessOpen(true)} className="levi-shell-cta hidden xl:inline-flex"><WalletCards className="h-4 w-4" />Connect LEVI</button>
          )}
        ...
        {open ? (
          <div id="levi-mobile-navigation" className="levi-mobile-panel xl:hidden">
            <div className="levi-container py-4">
              ...
              {auth.session || auth.walletAddress ? <button type="button" onClick={() => void handleLogout()} className="levi-shell-cta mt-3 flex w-full justify-center"><LogOut className="h-4 w-4" />Logout</button> : <button type="button" onClick={() => { setOpen(false); setAccessOpen(true); }} className="levi-shell-cta mt-3 flex w-full justify-center"><WalletCards className="h-4 w-4" />Connect LEVI</button>}
            </div>
          </div>
        ) : null}
      </header>
